...
<template>
	<div class="change_opwd_main">
		<div class="change_opwd_main_title">
			<!-- 修改资金密码 -->
			<!-- 法币交易、提币使用 -->
			<h2 class="title" style="font-size: 24px;">{{$t('Gic.securityChangeTpwd[0]')}}<span style="font-size: 14px;color: #999999;margin-left: 20px;">{{$t('Gic.securityChangeTpwd[1]')}}</span></h2>
		</div>
		<div class="form_warp tpwd">
			<div class="form_cont">
				<el-form :label-position="labelPosition" :model="userData" :rules="changeTpwdFormRules" ref="changeTpwdForm" label-width="100px" class="demo-ruleForm">
					<!-- 旧资金密码 -->
					<el-form-item :label="$t('Gic.securityChangeTpwd[2]')" prop="old_password">
						<el-input type="password" v-model="userData.old_password" :placeholder="$t('Gic.securityChangeopwd[3]')" auto-complete="off"></el-input>
					</el-form-item>
					<!-- 新资金密码 -->
					<el-form-item :label="$t('Gic.securityChangeTpwd[3]')"  prop="password">
						<el-input type="password" v-model="userData.password" :placeholder="$t('Gic.securityChangeopwd[3]')" auto-complete="off"></el-input>
					</el-form-item>
					<!-- 确认资金密码 -->
					<el-form-item :label="$t('Gic.securityChangeTpwd[4]')"  prop="password_confirmation">
					...
					<!-- 邮箱地址 -->
					<el-form-item :label="$t('Gic.securityChangeTpwd[5]')">
						<el-input disabled type="text" v-model="email" auto-complete="off"></el-input>
					</el-form-item>
					<!-- 获取验证码 -->
					<div class="code_warp">
						<el-form-item :label="$t('Gic.securityChangeTpwd[6]')" prop="code">
							<!-- 请输入验证码 -->
							<el-input v-model.number="userData.code" :placeholder="$t('Gic.securityChangeTpwd[7]')" auto-complete="off"></el-input>
						</el-form-item>
					...
					<el-form-item>
						<el-button type="primary" @click="submitForm('userData')" :loading="loading">{{$t('Gic.securityChangeopwd[7]')}}</el-button><!-- 确定 -->
					</el-form-item>
				</el-form>
			</div>
...
<script>
	export default {
		...
		methods: {
			...
			submitForm() {
				var _this = this;
				_this.$refs.changeTpwdForm.validate((valid) => {
					if(valid) {
						_this.loading = true; //防止表单重复提交标志
						_this.userData.old_password = _this.$public.$md5(_this.userData.old_password);
						...
						_this.$http.post(_this.$http.resetPaymentPassword, _this.userData).then(function(response) {
		...
		created: function() {
			var _this = this;
			_this.email = this.$public.phoneEncryption(sessionStorage.userData);
