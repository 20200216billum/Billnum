...
MAIndicator.prototype.__construct = function () {
    ...
    var d = new ParameterExpr("M3", 2, 1000, 20);
...
//  this._colors[Theme.Color.Negative] = "#00ba53";//跌价 绿色
    this._colors[Theme.Color.Positive] = "#00ba53";//涨价 红色
    this._colors[Theme.Color.Negative] = "#FF3232";//跌价 绿色
...
//  this._colors[Theme.Color.Negative] = "#53b37b";//绿色，跌价
    this._colors[Theme.Color.Positive] = "#53b37b";//红色，涨价
    this._colors[Theme.Color.Negative] = "#db5542";//绿色，跌价
...
function KLineMouseEvent() {
    $(document).ready(function () {
        ...
        $("#chart_container .chart_toolbar_tabgroup a").click(function () {
            ...
            location.href = typehtml+"?time="+$(this).parent().attr("data_name");
            data.datas.data = [];
        });
...
var refresh_handler = setInterval(refresh_function, 3000);
...
function clear_refresh_counter() {
    ...
    refresh_handler = setInterval(refresh_function, 20000)
}

var TimeTransformation = {};
// 时间戳转换日期格式
TimeTransformation.getYMD = function(data){
    var d = new Date(data);  
...
// 日期格式转换为时间戳
TimeTransformation.getTIME = function(data,str){
    var d = new Date(data + " " + str);  
    var timedata = d.getTime();
   
    return timedata; 
}
var fjsadfah = TimeTransformation.getYMD(1520524800000);
// 开始数据
var data = { "des": "注释", "isSuc": true, "datas": { "USDCNY": 6.83, "contractUnit": "", "data": [], "marketName": "币柜网", "moneyType": "stcoktype", "symbol": "btc38btccny", "url": "官网地址", "topTickers": [] } }
// //上一次收盤價
var lastClosePrice = 0;
var flats = true;
// 数据处理
var DataProcessing = {}
DataProcessing.goodsHistoryInfoGet = function(result){
    var arrayshuju = [];
    if (data.datas.data.length =="0") {
        for (var i = 0; i < result.data.length; i++) {
            arrayshuju = [];
            var datatime = TimeTransformation.getYMD(result.data[i].date);
            //当前时间
            arrayshuju[0] = TimeTransformation.getTIME(datatime,result.data[i].time);
            // 开盘价
            arrayshuju[1] = Number(result.data[i].openingPrice);
            // 最高价
            arrayshuju[2] = Number(result.data[i].highestPrice);
            // 最低价
            arrayshuju[3] = Number(result.data[i].lowestPrice);
            // 收盘价
            arrayshuju[4] = Number(result.data[i].closingPrice);
            // 成交量
            arrayshuju[5] = Number(result.data[i].volume);
            data.datas.data.unshift(arrayshuju);
        };
    }else {
        var timepanduan = TimeTransformation.getYMD(result.data[0].date);
        if(data.datas.data[0][0] == TimeTransformation.getTIME(timepanduan,result.data[0].time)){
            return false; 
        }else {
            data.datas.data = [];
            arrayshuju = [];
            var datatime = TimeTransformation.getYMD(result.data[0].date);
            //当前时间
            arrayshuju[0] = TimeTransformation.getTIME(datatime,result.data[0].time);
            // 开盘价
            arrayshuju[1] = Number(result.data[0].openingPrice);
            // 最高价
            arrayshuju[2] = Number(result.data[0].highestPrice);
            // 最低价
            arrayshuju[3] = Number(result.data[0].lowestPrice);
            // 收盘价
            arrayshuju[4] = Number(result.data[0].closingPrice);
            // 成交量
            arrayshuju[5] = Number(result.data[0].volume);
            data.datas.data.unshift(arrayshuju);
        }
    };
}
...
var RequestData = function(showLoading) {
    ...
    if (data.datas.data.length == "0" && flats == true) {
    	flats = false
        $.get(GLOBAL_VAR.url,{
            goodsType:timestcok,
            code:stcoktype,
            ...
            if (result.code == "200") {
                DataProcessing.goodsHistoryInfoGet(result);
                hunzhituxiang(data.datas.data);
            }
        })
    }else {
        $.get(GLOBAL_VAR.url,{
    ...
    function hunzhituxiang(datashuju){
        GLOBAL_VAR.KLineData = datashuju;
...
function setHttpRequestParam(b, c, a, e) {
    ...
    var d = "action=goodsHistoryInfoGet&code=ETC_USDT&goodsType=" + b;
    return d
}
...
function switch_period(a) {
    ...
    GLOBAL_VAR.requestParam = setHttpRequestParam(a, GLOBAL_VAR.time_type, null, a.toString())
    ChartManager.getInstance().showCursor();
    calcPeriodWeight(a);
    if (a == "line") {
