...
DataProcessing.goodsHistoryInfoGet = function(result){
    ...
    if (data.datas.data.length =="0") {
        for (var i = 0; i < result.data.length; i++) {
            arrayshuju = [];
            //当前时间
            arrayshuju[0] = Number(result.data[i].date);
            // 开盘价
            arrayshuju[1] = Number(result.data[i].openPrice);
            // 最高价
            arrayshuju[2] = Number(result.data[i].high);
            // 最低价
            arrayshuju[3] = Number(result.data[i].low);
            // 收盘价
            arrayshuju[4] = Number(result.data[i].closePrice);
            // 成交量
            arrayshuju[5] = Number(result.data[i].volume);
            data.datas.data.unshift(arrayshuju);
        };
    }else {
        if(data.datas.data[0][0] == result.data[0].data){
            return false; 
        }else {
            data.datas.data = [];
            arrayshuju = [];
            //当前时间
            arrayshuju[0] = Number(result.data[i].data);
            // 开盘价
            arrayshuju[1] = Number(result.data[0].openPrice);
            // 最高价
            arrayshuju[2] = Number(result.data[0].high);
            // 最低价
            arrayshuju[3] = Number(result.data[0].low);
            // 收盘价
            arrayshuju[4] = Number(result.data[0].closePrice);
            // 成交量
...
function mouseWheel(a, b) {
    // console.log(ChartManager.getInstance())
